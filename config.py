...
# 검색 중복 제거 — 같은 영상에서 이 초 이내 구간은 '같은 상황'으로 보고 최고점 1개만(시간근접 dedup).
SEARCH_DEDUP_GAP_S = float(os.environ.get("SEARCH_DEDUP_GAP_S", "20"))
...
# ── 행동 이벤트 메모리 — '사람 행동/이벤트' 렌즈로 5초 구간을 캡션·분류(화재는 별도 열화상 담당). ──
# 구간 묘사 프롬프트 — 캡션(묘사)만 생성. 유형은 임베딩 분류기(classifier)가 별도로 붙인다.
